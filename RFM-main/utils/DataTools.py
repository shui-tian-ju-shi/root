...
class imgdataset(VisionDataset):
    def __init__(self, rootlist, process=None, transform=None, randomdrop=0):
        ...
        for root, label in self.rootlist:
            imglist = os.listdir(root)#列出文件夹下所有文件的目录
            print("Loading %s" % (root), end="\r")
            for p in imglist:#加载所有图片
    ...
    def __getitem__(self, index):
        img, label = self.dataset[index]#真数据集里面的
        img = Image.open(img)
        img = np.array(img)#先将图片转换成数组
        img = cv2.resize(img, (256, 256))#设置图片的高宽
        img = Image.fromarray(img)#再将数组转化成图片
